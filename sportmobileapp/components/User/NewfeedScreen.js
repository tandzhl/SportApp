...
import { FlatList, Text, StyleSheet, View, Dimensions, Image, TextInput, ScrollView, Modal, TouchableOpacity, Alert } from "react-native";
...
const baseURL = 'http://192.168.1.136:8000/';
...
export default function NewfeedScreen() {
    ...
    const loadFeeds = async () =>{
        ...
        try {
            ...
            const res = await authApis(token).get(endpoints['newfeed'], { params,
                headers: {
                    'Authorization': `Bearer ${token}`,
                }
             });
