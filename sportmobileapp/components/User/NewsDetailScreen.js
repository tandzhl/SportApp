...
const NewsDetailScreen = ({ route }) => {
  ...
  const handleLike = async () => {
    try {
      ...
      await authApis(token).post(endpoints["newfeed-like"](newfeedId), {}, {
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      const token = await AsyncStorage.getItem("token");
      const response = await authApis(token).post(endpoints["comments"](newfeedId), { content: commentText }, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const refreshComments = async() => {
    try {
      const token = await AsyncStorage.getItem("token");
      const res = await authApis(token).get(`${endpoints["comments"](newfeedId)}?limit=5`, {
        headers: {
    ...
    try {
      ...
      if (token) {
        const [response, comments] = await Promise.all([
          authApis(token).get(endpoints["newfeed-details"](newfeedId), {
            headers: { Authorization: `Bearer ${token}` },
          }),
          authApis(token).get(`${endpoints["comments"](newfeedId)}?limit=5`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
  ...
  const renderComment = ({ item }) => {
    ...
    const handleDeleteComment = async (commentId)=> {
      try {
        ...
        await authApis(token).delete(endpoints["comment-detail"](commentId), {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
    ...
    const handleUpdateComment = async () => {
      ...
      try{
        const token = await AsyncStorage.getItem("token");
        if (!token) throw new Error("Token không tồn tại");
        await authApis(token).put(endpoints['comment-detail'](item.id), {content: editText}, {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        });
