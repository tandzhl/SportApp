...
const CheckOrderScreen = ({ navigation }) => {
    ...
    const loadUsers = async () => {
        if (offset >= 0) {
            ...
            try {
                ...
                const res = await authApis(token).get(`${endpoints['users-by-role']}?role=member&username=${q}&limit=${LIMIT}&offset=${offset}`);
    ...
    const loadUserOrders = async(userId) => {
        ...
        try {
            const token = await AsyncStorage.getItem("token");
            const res = await authApis(token).get(`${endpoints['orders']}?user_id=${userId}`);
            const data = res.data || [];
            setOrders(data)
        } catch (error) {
    ...
    const updatePaidStatus = async(orderId) => {
        try{
            const token = await AsyncStorage.getItem("token");
            const res = await authApis(token).patch(endpoints['order-update-paid'](orderId));
            const data = res.data;
