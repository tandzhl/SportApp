...
const EmployeeScreen = ({navigation}) => {
    ...
    const loadCoachs = async () => {
        try {
            setLoading(true);
            const token = await AsyncStorage.getItem('token');
            const instructorRes = await authApis(token).get(endpoints['users-by-role'], {
                params: {
                    role: 'coach',
                }
            });
    ...
    const loadClassTypes = async () => {
        try {
            const token = await AsyncStorage.getItem('token');
            const classTypeRes = await authApis(token).get(endpoints['categories'], {
                headers: {
    ...
    const loadSportClasses = async () => {
        try{
            const token = await AsyncStorage.getItem('token');
            const sportClassRes = await authApis(token).get(endpoints['sportclasses'], {
                headers: {
                    'Authorization': `Bearer ${token}`,
                }
            });

            const sportClassData = sportClassRes.data.results;
    ...
    const updateSchedule = async (scheduleId, updateData) => {
        try {
            const token = await AsyncStorage.getItem('token');
            const res = await authApis(token).put(endpoints['update-schedule'](scheduleId), updateData, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                }
            });
    ...
    const deleteSchedule = async (scheduleId) => {
        try {
            const token = await AsyncStorage.getItem("token");
            await authApis(token).delete(endpoints['delete-schedule'](scheduleId), {
                headers: {
                    'Authorization': `Bearer ${token}`,
                }
            });
    ...
    const handleAdd = () => {
        navigation.navigate("employee-stack", {screen: 'employee-add-schedule',});
    };
            ...
            <FlatList
                ...
                renderItem={({item}) => (
                    <View style={styles.scheduleItem}>
                        ...
                        <Text style={styles.dateText}>{ moment(item.datetime).format("HH:mm DD/MM/YYYY")} at {item.place}</Text>
                    </View>
                )}
                            ...
                            <View style={styles.inputGroup}>
                                ...
                                <TouchableOpacity onPress={showDatePicker} style={styles.input}>
                                    <Text style={styles.inputText}>
                                        {new Date(editedSchedule.datetime).toLocaleDateString()}
                                    </Text>
                                </TouchableOpacity>
                            </View>
                            <View style={styles.inputGroup}>
